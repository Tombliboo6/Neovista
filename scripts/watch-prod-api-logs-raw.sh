...
usage() {
  cat <<'EOF'
Usage: ./scripts/watch-prod-api-logs-raw.sh [--since "10 min ago"]

Streams the full neovista-api journal from production with automatic reconnects.
EOF
}
...
echo "Watching raw neovista-api logs from ${SSH_HOST}"
if [[ -n "${SINCE_VALUE}" ]]; then
...
while true; do
  if run_remote_journal "${SINCE_VALUE}"; then
    exit 0
  fi
